...
| `review_queue(reason, max_results, include_archive)` | 只读列出当前身份检索锚点较弱的旧记忆，由记忆主人自行决定是否调整 |
...
新写入会先经过候选自检：记忆类型必须合法，标题和主体需要可读，标签会去重并只保留 3~6 个有区分度的词，操作方法缺少步骤顺序时会提示写入者复核。候选文本中的密码、token、API key、私钥和 URL 认证参数会在落库前隐藏；这层只处理本次新候选，不会批量改写旧记忆。

旧记忆通过 `review_queue` 进入只读候选层：它只指出标题不可检索、类型或主体缺失、标签稀疏/过载、操作步骤不清楚等原因，并按当前 MCP 身份隔离。共享记忆不会因为“看得见”就被分配给某个身份审阅；候选层不会修改、重标、合并、归档或删除任何记忆。管理前端可通过 `/api/review-candidates` 获取同一份只读数据，供后续移动端审阅页使用。

可选的“多关键词漏检兜底”保留原模糊排序，只在原结果会漏掉、且至少两个有效查询词在标题、主体、标签或正文中被严格找到时介入。`/api/search` 会返回 `retrieval_strategy`、`matched_in`、`tokens_hit` 和 `token_coverage`，便于说明“为什么找到它”。查询包含“怎么、步骤、配置、操作”等意图时，还可对 `procedure` 做轻量加权。
...
当前只实现了“新候选入库前的格式与隐私校验”，还没有保存原始事件、持久候选队列或人工批准流。原始事件黑匣子、可延期审阅的候选队列和客户端自动注入 hook 仍适合放在下一阶段：它们需要幂等、失败重试、隐私授权和客户端兼容性一起设计。

### 离线检索评测

仓库自带一组完全虚构的数据，用来比较现行排序和候选参数，不读取生产记忆、不调用网络，也不会写入真实桶：

```bash
python tools/retrieval_benchmark.py
```

报告包含 Hit@1、Hit@5、MRR、无关 Top-1 比例、应为空结果的准确率、命中字段贡献和每条结果的检索策略。新增或调整权重时，应先扩充这组问题并保留“确实找不到时返回空”的样本。
