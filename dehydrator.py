...
11. **memory_kind(强制)**: 每个 item 只能选择以下一种类型:
   - fact: 可被核对的状态或信息。仅仅提到某个人, 不等于 relationship
   - procedure: 工具、配置、步骤或操作方法
   - commitment: 明确的约定、承诺或待办
   - preference: 跨场景相对稳定的偏好, 一次性的喜欢/讨厌用 episode
   - relationship: 记忆核心是双方关系、互动模式或共同经历, 而不是某人说出的一条事实
   - episode: 一次性的事件或经历
   - reflection: 感受、理解、判断或反思
   - desire: 尚未完成、持续牵引行动的愿望
   同一条同时含事实与感受时, 按它最值得被什么问题检索出来选择; 不要因为文字温柔就把技术事实标成 relationship/reflection。
12. **subject(强制)**: 这条记忆主要在说谁/什么, 20 字以内, 优先明确人名、项目名或对象名; 不要使用 [[]]。
...
tags 生成规则：总计 3~6 个。优先使用明确的人名、项目名、对象名、规则名和操作词；仅在用户可能用另一种固定叫法检索时增加 1~2 个别名。不要添加“生活”“情感”“系统”“游戏”等不能区分这条记忆的宽泛词。
安全规则：密码、API key、token、授权码、私钥及含认证信息的完整 URL 不得写入 content、source_excerpt、summary 或 tags；只记录“凭据存放位置/用途”并用 [REDACTED] 代替值。
...
4. tags: 3~6 个可区分本条记忆的核心检索词；最多补充 1~2 个固定别名，不要堆叠宽泛上位词
...
4. tags（关键词标签）：分两步生成，合并为一个数组：
   ...
   第二步—必要别名：仅补充 1~2 个用户确实可能使用的固定别名或缩写，不添加宽泛上位词和泛场景词
   两步合并为一个 tags 数组，总计 3~6 个
5. suggested_name（建议桶名）：10字以内的简短标题
6. memory_kind（记忆类型）：只能从以下值选择一个：
   fact=可核对的状态或信息；仅仅提到某个人，不等于 relationship
   procedure=工具/配置/操作方法
   commitment=明确的约定、承诺或待办
   preference=跨场景相对稳定的偏好；一次性的喜欢/讨厌用 episode
   relationship=记忆核心是双方关系、互动模式或共同经历，而不是某人说出的一条事实
   episode=一次性的事件或经历
   reflection=感受/理解/判断或反思
   desire=尚未完成、持续牵引行动的愿望
   同时含事实与感受时，按它最值得被什么问题检索出来选择；不要因为文字温柔就把技术事实标成 relationship/reflection
...
tags 生成规则：总计 3~6 个，只保留能区分本条记忆的核心词和必要别名；不要添加宽泛上位词或泛场景词。
安全规则：不得在输出中保留密码、API key、token、授权码、私钥或含认证信息的完整 URL；用 [REDACTED] 代替值，只保留用途和安全存放位置。
...
class Dehydrator:
    ...
    async def digest(self, content: str) -> list[dict]:
        ...
        Returns: [{"name", "content", "domain", "valence", "arousal", "tags",
                   "importance", "memory_kind", "subject"}, ...]
        """
        if not content or not content.strip():
            return []
